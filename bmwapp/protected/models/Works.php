...
class Works extends CActiveRecord
{
    ...
    public function getOneWork($array)
    {
        ...
        $sql = sprintf("SELECT `id`,`user_id`,`name`,`img_url`,`description`,`type`,`vote_num`,`review`,`update_time`,`create_time` FROM %s where %s limit 1",$this->tableName(),$where);
    ...
    public function getUpwork($array,$sel = '',$order)
    {
    	...
    	$sql = sprintf("SELECT `id`,`user_id`,`name`,`img_url`,`description`,`status`,`vote_num`,`update_time`,`create_time` FROM %s where  %s and `review`=0 order by %s limit 1",$this->tableName(),$where,$order);
        //echo $sql;
    	$res = Yii::app()->db->createCommand($sql)->queryRow();
        return $res;
    }
